...
assert withXwidgets -> !withMacport || (withGTK3 && webkitgtk) != null;

let
  emacs = (if withMacport then stdenv else stdenv).mkDerivation
    (lib.optionalAttrs nativeComp
      ...
      buildInputs = [ ncurses gconf libxml2 gnutls gettext jansson texinfo ]
