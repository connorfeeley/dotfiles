...
*** Create server
#+begin_src shell
hcloud server create --image ubuntu-20.04 --name h8tsner --type cpx11 --ssh-key GPG --user-data-from-file $DOTFIELD_DIR/nixos/machines/h8tsner/user-data.txt --datacenter ash-dc1 --without-ipv4
#+end_src
