...
mkdir -p ~/.ssh && cp /run/share/cfeeley/.ssh/id_* ~/.ssh/

mkdir -p ~/.config && cp -a /run/share/cfeeley/.config/nix/ ~/.config
...
# Disabled: possibly less change of nuking my home directory.
mkdir /mnt/home
