...
in
{
  home.packages = with pkgs; [
    ...
    hub # GitHub CLI
    glab # GitLab CLI
    lab # Another GitLab CLI
    tig
    git-crypt
    git-standup
    git-review # : tool to submit code reviews to Gerrit
    git-big-picture # : visualize Git repositories
    git-run # : 'gr': a tool for managing multiple git repositories (github:mixu/gr)

    # # Other potentially interesting packages:
    # git-machete #: github:VirtusLab/git-machete
    # gita #: github:nosarthur/gita
    # gitleaks #: github:zricethezav/gitleaks
  ...
  programs.git = {
    ...
    signing = lib.mkIf enableSigning {
      key = pgpPublicKey;
      signByDefault = true;
      signer = "${git-gpg-privacy}/bin/git-gpg-privacy";
    };
